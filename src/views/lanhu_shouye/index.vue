<template>
  <div class="page flex-col">
    <div class="box_1 flex-col">
      <div class="group_1 flex-row">
        <img
          class="image_1"
          referrerpolicy="no-referrer"
          src="./assets/img/SketchPng324753e574872571a405af18c2b0e75ebf63e0fdff2931a145e6d2a673196977.png"
        />
        <span class="text_1">小马笔记</span>
        <span class="text_2">功能</span>
        <span class="text_3">价格</span>
        <span class="text_4">下载</span>
        <div class="text-wrapper_1 flex-col"><span class="text_5">登录</span></div>
        <div class="text-wrapper_2 flex-col"><span class="text_6">注册</span></div>
      </div>
      <div class="group_2 flex-row justify-between">
        <div class="box_2 flex-col justify-between">
          <div class="text-wrapper_3">
            <span class="text_7">让</span>
            <span class="text_8">笔记</span>
            <span class="paragraph_1">
              更简单
              <br />
              让知识更加
            </span>
            <span class="text_9">智慧</span>
          </div>
          <span class="paragraph_2">
            一款开源&nbsp;·&nbsp;本地优先&nbsp;·&nbsp;云同步&nbsp;·&nbsp;AI&nbsp;思考的笔记应用支持块笔记&nbsp;·&nbsp;手写&nbsp;·&nbsp;白板&nbsp;
            <br />
            ·&nbsp;PDF阅读助力个人与团队构建高效的知识系统
          </span>
        </div>
        <img
          class="image_2"
          referrerpolicy="no-referrer"
          src="./assets/img/SketchPng5251923674fc8951153d1fd5b7dd66d9d19f7101666e8b0ac181a1d323f83001.png"
        />
      </div>
      <div class="group_3 flex-row justify-between">
        <div class="group_4 flex-row">
          <div class="image-text_1 flex-row justify-between">
            <img
              class="label_1"
              referrerpolicy="no-referrer"
              src="./assets/img/SketchPngdb7d8ee97b465d10531a7b9a6295e7f829c13b83879aa76b139e04c44edf9675.png"
            />
            <span class="text-group_1">开始注册</span>
          </div>
        </div>
        <div class="group_5 flex-row">
          <div class="image-text_2 flex-row justify-between">
            <img
              class="label_2"
              referrerpolicy="no-referrer"
              src="./assets/img/SketchPng549150edb39248e89729f2203af0ba0175b292928380b7693d346ed1b0f55d49.png"
            />
            <span class="text-group_2">立即下载</span>
          </div>
        </div>
      </div>
      <img
        class="image_3"
        referrerpolicy="no-referrer"
        src="./assets/img/SketchPng10bc3654478fe91a3290fcd96730bfb6e89a75485f4b6a850381bdc727efbad1.png"
      />
      <div class="group_6 flex-col"></div>
    </div>
    <div class="box_3 flex-col">
      <div class="text-wrapper_4">
        <span class="text_10">易于使用</span>
        <span class="text_11">且功能强大</span>
      </div>
      <div class="text-wrapper_5 flex-row">
        <span class="text_12">模块化笔记</span>
        <span class="text_13">多维表</span>
        <span class="text_14">任务</span>
        <span class="text_15">模版</span>
        <span class="text_16">站点</span>
      </div>
      <div class="box_4 flex-col"><div class="box_5 flex-col"></div></div>
      <div class="text-group_3 flex-col justify-between">
        <span class="text_17">模块化笔记</span>
        <span class="text_18">一键插入文字、表格.代码、音视频、网页内容。满足多场采创作需</span>
      </div>
      <div class="box_6 flex-col"></div>
      <div class="text-wrapper_6">
        <span class="text_19">独一无二</span>
        <span class="text_20">的手写体验</span>
      </div>
      <div class="text-wrapper_7 flex-row justify-between">
        <span class="text_21">手写笔记</span>
        <span class="text_22">纸张</span>
        <span class="text_23">工具</span>
        <span class="text_24">书写</span>
        <span class="text_25">画图</span>
      </div>
      <div class="box_7 flex-col"><div class="box_8 flex-col"></div></div>
      <div class="box_9 flex-row">
        <div class="text-wrapper_8 flex-col justify-between">
          <span class="text_26">手写笔记</span>
          <span class="text_27">支持流程手写，让记录不再受限于键盘和线性输入</span>
        </div>
        <div class="group_7 flex-col"></div>
      </div>
      <div class="text-wrapper_9">
        <span class="text_28">一起思维发散，</span>
        <span class="text_29">打破页面限制</span>
      </div>
      <div class="text-wrapper_10 flex-row justify-between">
        <span class="text_30">白板页面</span>
        <span class="text_31">书写</span>
        <span class="text_32">文字</span>
        <span class="text_33">图片</span>
        <span class="text_34">协同</span>
      </div>
      <div class="box_10 flex-col"><div class="section_1 flex-col"></div></div>
      <span class="text_35">白板页面</span>
      <span class="text_36">手绘直线、箭头、方框圆形，自动识别为规范图形，助你轻松绘制结构图、流程图和草图。</span>
      <div class="box_11 flex-col"></div>
      <div class="box_12 flex-row">
        <div class="group_8 flex-col">
          <div class="text-wrapper_11">
            <span class="text_37">AI&nbsp;也能个性化</span>
            <span class="text_38">，自选模型，更懂你</span>
          </div>
          <div class="text-wrapper_12 flex-row justify-between">
            <span class="text_39">书写</span>
            <span class="text_40">文字</span>
            <span class="text_41">图片</span>
            <span class="text_42">协同</span>
          </div>
          <span class="text_43">选择AI</span>
        </div>
        <div class="group_9 flex-col">
          <span class="text_44">多种选择</span>
          <div class="group_10 flex-col"><div class="group_11 flex-col"></div></div>
        </div>
      </div>
      <span class="text_45">
        小马笔记支持多个主流大模型，你可以按任务自由选择:总结、翻译写作、问答，用最合适的&nbsp;AI，做最懂你的事
      </span>
      <div class="box_13 flex-col"></div>
      <div class="box_14 flex-col"></div>
    </div>
    <div class="box_15 flex-col">
      <div class="text-wrapper_13">
        <span class="text_46">专为</span>
        <span class="text_47">重视隐私</span>
        <span class="text_48">的人打造</span>
      </div>
      <div class="list_1 flex-row">
        <div class="list-items_1 flex-col" v-for="(item, index) in loopData0" :key="index">
          <img class="image_4" referrerpolicy="no-referrer" :src="item.lanhuimage0" />
          <div class="text-group_4 flex-col justify-between">
            <span class="text_49" v-html="item.lanhutext0"></span>
            <span class="text_50" v-html="item.lanhutext1"></span>
          </div>
        </div>
      </div>
      <div class="text-wrapper_14">
        <span class="text_51">利用API</span>
        <span class="text_52">让笔记进行联动</span>
      </div>
      <div class="text-wrapper_15 flex-col">
        <span class="paragraph_3">
          xiaomabiji&nbsp;APl
          <br />
          <br />
          POST&nbsp;.https://xiac1010日skey/lication/jsonContent-type:&nbsp;app
          <br />
          "Hello，.#xiaomabiji“conte!it”xiaomabijihttps://.com
          <br />
          ]
        </span>
      </div>
      <div class="text-wrapper_16">
        <span class="text_53">全平台</span>
        <span class="text_54">同步，即可下载</span>
      </div>
      <div class="block_1 flex-row justify-between">
        <div class="image-text_3 flex-row justify-between">
          <img
            class="label_3"
            referrerpolicy="no-referrer"
            src="./assets/img/SketchPng08694ad54a34bc70d724456830b35fd041f18bc36c556c4487329fc5ae8d44a7.png"
          />
          <span class="text-group_5">iPhone</span>
        </div>
        <div class="image-text_4 flex-row justify-between">
          <img
            class="label_4"
            referrerpolicy="no-referrer"
            src="./assets/img/SketchPng7a6d5f11f5147f0bb46abe86b21e01ad698bf2114829d45a4bbf5604260b05bc.png"
          />
          <span class="text-group_6">iPad</span>
        </div>
        <div class="image-text_5 flex-row justify-between">
          <img
            class="label_5"
            referrerpolicy="no-referrer"
            src="./assets/img/SketchPng1b4e8db79667fa506121897b908f2d40e82302b9a5cef361d9525f76e3eaa2dc.png"
          />
          <span class="text-group_7">Windows</span>
        </div>
        <div class="image-text_6 flex-row justify-between">
          <img
            class="label_6"
            referrerpolicy="no-referrer"
            src="./assets/img/SketchPng0672c9c1ed19cd7a3f53ebf62b0ba8b32e8d7f13b112be8f22a3124d2360e174.png"
          />
          <span class="text-group_8">MacOS</span>
        </div>
        <div class="image-text_7 flex-row justify-between">
          <img
            class="label_7"
            referrerpolicy="no-referrer"
            src="./assets/img/SketchPng9afbd0358f6c071d449fb3bcf93227e4b4182f9c0e8ebb300595df03151523dd.png"
          />
          <span class="text-group_9">Android</span>
        </div>
        <div class="image-text_8 flex-row justify-between">
          <img
            class="label_8"
            referrerpolicy="no-referrer"
            src="./assets/img/SketchPng102be35cf9affa77789ed786d7e7c2bc29a4cba51bdd35acbf56c09188aeb89d.png"
          />
          <span class="text-group_10">浏览器</span>
        </div>
      </div>
    </div>
    <div class="box_16 flex-col">
      <div class="text-wrapper_17 flex-row">
        <span class="text_55">产品</span>
        <span class="text_56">资源</span>
        <span class="text_57">联系我们</span>
      </div>
      <div class="section_2 flex-row">
        <div class="box_17 flex-col justify-between">
          <div class="image-text_9 flex-row justify-between">
            <img
              class="image_5"
              referrerpolicy="no-referrer"
              src="./assets/img/SketchPng48f12606e84bdac2142ad0dcc1ec6cdce8a478bf51ea160eaec6f4f395adf276.png"
            />
            <span class="text-group_11">小马笔记</span>
          </div>
          <span class="text_58">京ICP备2025131868号</span>
        </div>
        <div class="text-wrapper_18 flex-col justify-between">
          <span class="text_59">桌面端</span>
          <span class="text_60">手机端</span>
          <span class="text_61">隐私协议</span>
          <span class="text_62">用户协议</span>
        </div>
        <div class="text-wrapper_19 flex-col justify-between">
          <span class="text_63">使用教程</span>
          <span class="text_64">快速上手</span>
          <span class="text_65">常见问题</span>
        </div>
        <div class="text-wrapper_20 flex-col justify-between">
          <span class="text_66">B站</span>
          <span class="text_67">抖音</span>
          <span class="text_68">小红书</span>
          <span class="text_69">邮箱</span>
        </div>
      </div>
    </div>
  </div>
</template>
<script>
export default {
  data() {
    return {
      loopData0: [
        {
          lanhuimage0:
            'https://lanhu-oss-proxy.lanhuapp.com/SketchPng2e7707cf66755335a37ed237668e0c61fd5d40800e8380dc21d124dada741b8a',
          lanhutext0: '本地优先',
          lanhutext1: '数据完全归你所有<br/>你写的每一笔<br/>只有你能看到'
        },
        {
          lanhuimage0:
            'https://lanhu-oss-proxy.lanhuapp.com/SketchPng74d33164d39b4c1c7fab3f261a5e23c553be57427699d88c032e835f482775c8',
          lanhutext0: '完全开源',
          lanhutext1: '透明可信全程加密<br/>传输源代码公开<br/>逻辑透明安全看得见'
        },
        {
          lanhuimage0:
            'https://lanhu-oss-proxy.lanhuapp.com/SketchPng3dc8ee3fbeb910e54c22a21cc961bf69825a740c8bdd7545f58833c26aef055c',
          lanhutext0: '数据可随时导出',
          lanhutext1: '不设限想迁移就迁移<br/>不搞特殊格式<br/>你永远是笔记的主人'
        }
      ],
      constants: {}
    };
  },
  methods: {}
};
</script>
<style scoped lang="less" src="./assets/index.less" />